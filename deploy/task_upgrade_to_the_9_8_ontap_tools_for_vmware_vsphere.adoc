...
NOTE: A partir da atualização do ONTAP Tools 9.12, todos os processos de autenticação e comunicação dos sistemas de armazenamento foram alterados, passando da autenticação básica para a autenticação baseada em certificado, confiando automaticamente nos certificados de armazenamento do ONTAP .
